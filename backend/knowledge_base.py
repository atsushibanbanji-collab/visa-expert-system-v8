...
# 注意: idとnameはactionから自動生成されるため、引数として渡さない
_HARDCODED_RULES: List[Rule] = [
    # ========== Eビザ関連ルール ==========
    Rule(
        conditions=["申請者と会社の国籍が同じです", "会社がEビザの条件を満たします", "申請者がEビザの条件を満たします"],
        action="Eビザでの申請ができます",
        rule_type=RuleType.MIDDLE,
        visa_type="E"
    ),
    Rule(
        conditions=["会社がEビザの投資の条件を満たします", "会社がEビザの貿易の条件を満たします"],
    ...
    Rule(
        conditions=["減価償却前の設備や建物が30万ドル以上財務諸表の資産に計上されています", "30万ドル以上で企業を買収した会社か、買収された会社です", "まだ十分な売り上げがなく、これまでに人件費などのランニングコストを含め、30万ドル以上支出しています", "会社設立のために、30万ドル以上支出しました（不動産を除く）"],
    ...
    Rule(
        conditions=["会社の行う貿易の50％が日米間です", "会社の行う貿易は継続的です", "貿易による利益が会社の経費の80％以上をカバーしています"],
        action="会社がEビザの貿易の条件を満たします",
        rule_type=RuleType.INITIAL,
        visa_type="E"
    ),
    Rule(
        conditions=["申請者がEビザのマネージャー以上の条件を満たします", "申請者がEビザのスタッフの条件を満たします", "EビザTDY(short-term needs)の条件を満たします"],
    ...
    Rule(
        conditions=["米国拠点でEビザでマネージャー以上として認められるポジションに就きます", "マネージャー以上のポジションの業務を遂行する十分な能力があります"],
        action="申請者がEビザのマネージャー以上の条件を満たします",
        rule_type=RuleType.MIDDLE,
        visa_type="E"
    ),
    Rule(
        conditions=["CEOなどのオフィサーのポジションに就きます", "経営企画のマネージャーなど、米国拠点の経営に関わるポジションに就きます", "評価・雇用に責任を持つ複数のフルタイムのスタッフを部下に持つマネージャー以上のポジションに就きます"],
    ...
    Rule(
        conditions=["米国拠点のポジションの業務に深く関連する業務の経験が2年以上あります", "マネジメント経験が2年以上あります"],
        action="マネージャー以上のポジションの業務を遂行する十分な能力があります",
        rule_type=RuleType.MIDDLE,
        visa_type="E"
    ),
    Rule(
        conditions=["2年以上のマネージャー経験があります", "マネジメントが求められるプロジェクトマネージャーなどの2年以上の経験があります"],
    ...
    Rule(
        conditions=["理系の大学院卒で、米国拠点の技術系の業務に深く関連する3年以上の業務経験があります", "理系の学部卒で、米国拠点の技術系の業務に深く関連する4年以上の業務経験があります", "米国拠点の業務に深く関連する5年以上の業務経験があります"],
    ...
    Rule(
        conditions=["2年以内の期間で、目的を限定した派遣理由を説明できます", "米国拠点の業務に深く関連する2年以上の業務経験があります"],
    ...
    # ========== Lビザ関連ルール ==========
    Rule(
        conditions=["アメリカ以外からアメリカへのグループ内での異動です", "会社がBlanket Lビザの条件を満たします", "申請者がBlanket Lビザの条件を満たします"],
        action="Blanket Lビザでの申請ができます",
        rule_type=RuleType.MIDDLE,
        visa_type="L"
    ),
    Rule(
        conditions=["アメリカにある子会社の売り上げの合計が25百万ドル以上です", "アメリカにある子会社が1,000人以上ローカル採用をしています", "1年間に10人以上Lビザのペティション申請をしています"],
    ...
    Rule(
        conditions=["直近3年のうち1年以上、アメリカ以外のグループ会社に所属していました", "Blanket Lビザのマネージャーまたはスタッフの条件を満たします"],
        action="申請者がBlanket Lビザの条件を満たします",
        rule_type=RuleType.MIDDLE,
        visa_type="L"
    ),
    Rule(
        conditions=["Blanket Lビザのマネージャーの条件を満たします", "Blanket Lビザスタッフの条件を満たします"],
    ...
    Rule(
        conditions=["マネージャーとしての経験があります", "アメリカでの業務はマネージャーとみなされます"],
        action="Blanket Lビザのマネージャーの条件を満たします",
        rule_type=RuleType.INITIAL,
        visa_type="L"
    ),
    Rule(
        conditions=["specialized knowledgeがあります", "アメリカでの業務はspecialized knowledgeを必要とします"],
        action="Blanket Lビザスタッフの条件を満たします",
        rule_type=RuleType.INITIAL,
        visa_type="L"
    ),
    Rule(
        conditions=["アメリカ以外からアメリカへのグループ内での異動です", "申請者がLビザ（Individual）の条件を満たします"],
        action="Lビザ（Individual）での申請ができます",
        rule_type=RuleType.MIDDLE,
        visa_type="L"
    ),
    Rule(
        conditions=["直近3年のうち1年以上、アメリカ以外のグループ会社に所属していました", "Lビザ（Individual）のマネージャーまたはスタッフの条件を満たします"],
        action="申請者がLビザ（Individual）の条件を満たします",
        rule_type=RuleType.MIDDLE,
        visa_type="L"
    ),
    Rule(
        conditions=["マネージャーとしての経験があります", "アメリカでの業務はマネージャーとみなされます", "アメリカでは大卒、フルタイムの部下が2名以上います"],
    ...
    Rule(
        conditions=["Lビザ（Individual）のマネージャーの条件を満たします", "Lビザ（Individual）のスタッフの条件を満たします"],
    ...
    # ========== Bビザ関連ルール ==========
    Rule(
        conditions=["Bビザの申請条件を満たす（ESTAの認証は通る）", "Bビザの申請条件を満たす（ESTAの認証は通らない）"],
        action="Bビザの申請ができます",
        rule_type=RuleType.MIDDLE,
        is_or_rule=True,
        visa_type="B"
    ...
    Rule(
        conditions=["アメリカでの活動は商用の範囲です", "1回の滞在期間は90日を越えます", "1回の滞在期間は6か月を越えません"],
        action="Bビザの申請条件を満たす（ESTAの認証は通る）",
        rule_type=RuleType.INITIAL,
        visa_type="B"
    ),
    Rule(
        conditions=["アメリカでの活動は商用の範囲です", "1回の滞在期間は6か月を越えません"],
        action="Bビザの申請条件を満たす（ESTAの認証は通らない）",
        rule_type=RuleType.INITIAL,
        visa_type="B"
    ),
    Rule(
        conditions=["アメリカの会社に販売した装置や設備のための作業をします", "装置や設備の販売を示す契約書や発注書があります", "1回の滞在期間は6か月を越えません"],
        action="契約書に基づくBビザの申請ができます",
        rule_type=RuleType.INITIAL,
        visa_type="B"
    ),
    Rule(
        conditions=["H-1Bビザが必要な専門性の高い作業をします", "1回の滞在期間は6か月を越えません"],
        action="B-1 in lieu of H-1Bビザの申請ができます",
        rule_type=RuleType.INITIAL,
        visa_type="B"
    ),
    Rule(
        conditions=["研修内容は商用の範囲です", "研修期間は６か月以内です"],
        action="Bビザの申請ができます",
        rule_type=RuleType.INITIAL,
        visa_type="B"
    ),
    ...
    # ========== H-1Bビザ関連ルール ==========
    Rule(
        conditions=["大卒以上で、専攻内容と業務内容が一致しています", "大卒以上で、専攻内容と業務内容が異なりますが、実務経験が3年以上あります", "大卒以上ではありませんが、実務経験が(高卒は12年以上、高専卒は3年以上）あります"],
    ...
    # ========== J-1ビザ関連ルール ==========
    Rule(
        conditions=["研修にOJTが含まれます", "研修期間は18か月以内です", "申請者に研修に必要な英語力はあります"],
